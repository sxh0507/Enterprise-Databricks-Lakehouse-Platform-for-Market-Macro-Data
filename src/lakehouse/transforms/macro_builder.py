...
def build_gold_market_macro_daily(
    ...
    mkt = mkt_df.groupBy("source", "symbol", F.to_date("bar_start_ts").alias("trade_date")).agg(
        F.max_by("close", "bar_end_ts").alias("close_px"),
    ...
    fx = fx_df.filter((F.col("base_ccy") == "EUR") & (F.col("quote_ccy") == "USD")).select(
        F.col("rate_date"),
        F.col("fx_rate").alias("raw_fx")
    )
    
    # 3. FED Data (DFF)
    fed = fed_df.filter(F.col("series_id") == "DFF").select(
        F.col("obs_date"),
        F.col("value").alias("raw_fed")
    )
    
    # Join everything together
    joined = mkt.join(fx, mkt.trade_date == fx.rate_date, "left") \
                .join(fed, mkt.trade_date == fed.obs_date, "left")
