...
def test_resolve_days_to_fetch_backfill():
    ingestor = CryptoBronzeIngestor(
        spark=None, catalog="cat", schema="sch", table_name="tbl", api_client=None
    )
...
def test_resolve_days_to_fetch_realtime_symbol_casing(monkeypatch):
    ...
    monkeypatch.setattr(
        CryptoBronzeIngestor,
        "_get_watermarks",
        lambda self, symbols, interval: {
            "BTCUSDT": datetime(2026, 3, 2, 0, 0, tzinfo=timezone.utc)
        },
    )
